...
#ifdef R
int getlength(SEXP inp) { return Rf_length(inp); }
...
public:
  ...
  VEC(const long unsigned int n) : subsetted(0), ismatrix(0), d(n), temp(1) {
  ...
  // VEC(const R& mat, int nrows_, int ncols_) : d(mat), subsetted(0),
  // ncols(ncols_), nrows(nrows_), ismatrix(0) {}
  ...
  VEC(const int rows, const int cols)
      : subsetted(0), ismatrix(1), nrows(rows), ncols(cols), d(rows * cols),
        temp(1) {}
  ...
  // VEC(const int n, const T* ptr, int cob) : d(n, ptr, cob), subsetted(0),
  // ismatrix(0) {} //cob = copy, owning, borrow
  VEC(const int r, const int c, T *ptr, int cob)
      : subsetted(0), ismatrix(1), nrows(r), ncols(c), d(r * c, ptr, cob),
        temp(1) {} // cob = copy, owning, borrow
  // VEC(const int r, const int c, const T* ptr, int cob) : d(r*c, ptr, cob),
  // subsetted(0), ismatrix(1), nrows(r), ncols(c) {} //cob = copy, owning,
  // borrow
  ...
  */
  ...
  operator double() const{
    ass(this -> size() == 1, "Try tp convert object to double which is non
  scalar"); return d[0];
  }
  */
  ...
  operator SEXP() const {
    ...
    if (this->im() == false) {
      ...
      // SEXP M = PROTECT(Rf_allocVector(REALSXP, d.size()) );//
      // PROTECT(Rf_allocMatrix(REALSXP, nrows, ncols));
  ...
  VEC &operator=(const VEC &other_vec) {
    ...
    if (this->subsetted == false) {
      ...
      ass(indices.size() <= other_vec.size(),
...
int d2i(double inp) { return static_cast<int>(inp); }

double i2d(int inp) { return static_cast<double>(inp); }

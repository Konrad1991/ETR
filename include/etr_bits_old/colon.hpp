...
int length(double inp) { return 1; }

int length(int inp) { return 1; }

int length(bool inp) { return 1; }

template <typename T> int length(VEC<T> &inp) { return inp.size(); }

template <typename T> VEC<double> dim(const VEC<T> &inp) {
  if (inp.im() == false) {
